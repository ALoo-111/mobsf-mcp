...
The curl_cffi adapter uses browser impersonation (chrome124) on every request
to bypass Cloudflare managed challenges.  Set CURL_CFFI_IMPERSONATE to change
the profile or HTTP_CLIENT_BACKEND to fall back to requests/httpx.
"""
...
class CurlCffiBackend(HTTPBackend):
    """curl_cffi transport with per-request browser impersonation to bypass Cloudflare WAF.

    Uses ``impersonate="chrome124"`` on every request.  The profile can be overridden
    via the ``CURL_CFFI_IMPERSONATE`` environment variable.
    """
    ...
    def __init__(self, settings: Settings, headers: Mapping[str, str]) -> None:
        try:
            ...
            raise ConfigurationError(
                "HTTP_CLIENT_BACKEND=curl_cffi requires the curl-cffi package"
            ) from exc

        self._curl_requests = curl_requests
        # NOTE: curl_cffi AsyncSession does NOT accept impersonate in __init__.
        # impersonate must be passed per-request.
    ...
    async def request(
        ...
        *,
        ...
        # Build kwargs for curl_cffi request
        ...
        # impersonate MUST be passed per-request, not in session constructor
        if self._impersonate:
            request_kwargs["impersonate"] = self._impersonate

        try:
            response = await self._session.request(
                method, absolute_url, **request_kwargs
            )
        except Exception as exc:
            logger.error("curl_cffi request failed: %s", exc)
            raise
